...
interface ICurveRegistry {
    ...
    function get_underlying_coins(address _pool) external view returns (address[4] memory);
...
interface ICurve {
    ...
    function wrapped_coins(uint256 arg0) external view returns (address out);
...
interface ICurveCrypto {
    ...
    function calc_token_amount(uint256[2] memory _amounts, bool deposit) external view returns (uint256); // crypto factory registry
...
interface BaseLendingPool2Coins {
    function add_liquidity(
        uint256[2] memory amounts,
        uint256 min_mint_amount,
        bool _use_wrapped
